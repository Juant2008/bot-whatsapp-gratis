...
const INTERVALO_REVISION = 60000; 
const DELAY_ENTRE_MENSAJES = 8000; 
...
async function procesarFacturas(sock, pool) {
    if (!sock || !sock.user) {
        console.log("⏳ Notificador: Esperando conexión de WhatsApp...");
        return;
    }

    try {
        console.log(`🔍 ${new Date().toLocaleTimeString()} - Notificador: Revisando facturas...`);
        ...
        console.error("❌ Error en Notificador:", error.message);
    }
}
...
async function enviarNotificacionInmediata(sock, pool, factura) {
    try {
        ...
        if (cliente && vendedor) {
            ...
            console.log(`[Notificador] ✅ Factura #${factura.nro_factura} notificada.`);
        }
...
async function enviarNotificacionMora(sock, pool, factura) {
    try {
        ...
        if (cliente && vendedor) {
            ...
            console.log(`[Notificador] 🚩 Mora factura #${factura.nro_factura} notificada.`);
        }
