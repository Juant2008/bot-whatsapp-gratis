...
async function obtenerDetalleFacturas(ids) {
    ...
    try {
        ...
        const [rows] = await conn.query(
            ...
            AND anulado <> 'si' AND pagada = 'NO'`,
            formatIds
        );
        return rows;
    } catch (e) { 
        console.error("❌ Error DB detalle:", e.message); 
        return []; 
...
async function ejecutarEnvioMasivo(sock, deudores) {
    ...
    for (const row of deudores) {
        ...
        } catch (e) { console.error("Error envío unitario:", e.message); }
    }
    console.log("🏁 Proceso de cobranza terminado.");
}
